...
class InvDict(MutableMapping[KT, VT]):
    ...
    Code inspired from:
    https://github.com/mCodingLLC/VideosSampleCode/blob/master/videos/124_abc_collections
    """
    ...
    def __init__(
        ...
        if forward is None:
            ...
            # Inverse init (private)
            self._forward = forward
            # Private param, assume correct
            self._backward = _backward
        else:
            # User-provided forward init
            self._forward = forward
            self._backward = {
    ...
    def _raise_non_invertible(self, key1: KT, key2: KT, value: VT):
        raise ValueError(
            f"non-invertible: {key1}, {key2} both map to: {value}"
        )
    ...
    def clear(self) -> None:
        """Remove all items from the InvDict."""
